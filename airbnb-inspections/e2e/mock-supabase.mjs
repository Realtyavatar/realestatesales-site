...
// Storage) — just enough to drive Checkout Inspections end-to-end for
// verification. No real Supabase project needed.
import http from "node:http";
import crypto from "node:crypto";

const PORT = 54321;
const USER = { email: "host@example.com", password: "test-password-123" };
...
const defaults = {
  inspections: () => ({
    id: uuid(), property_name: "", property_address: "",
    inspection_type: "checkout", status: "in_progress", notes: "",
    started_at: now(), completed_at: null,
    created_at: now(), updated_at: now(),
  }),
  rooms: () => ({
    id: uuid(), inspection_id: null, room_type: "", name: "",
    checklist: [], sort_order: 0, created_at: now(), updated_at: now(),
  }),
  photos: () => ({
    id: uuid(), inspection_id: null, room_id: null, storage_path: "",
    caption: "", taken_at: now(), sort_order: 0, created_at: now(),
  }),
  damage_flags: () => ({
    id: uuid(), inspection_id: null, room_id: null, description: "",
    severity: "minor", notes: "", created_at: now(), updated_at: now(),
  }),
};

const db = { inspections: [], rooms: [], photos: [], damage_flags: [] };
    ...
    if (req.method === "DELETE") {
      ...
      if (table === "inspections") {
        for (const i of matched) {
          db.rooms = db.rooms.filter((r) => r.inspection_id !== i.id);
          db.photos = db.photos.filter((p) => p.inspection_id !== i.id);
          db.damage_flags = db.damage_flags.filter((f) => f.inspection_id !== i.id);
        }
      }
      if (table === "rooms") {
        for (const r of matched) {
          db.photos = db.photos.filter((p) => p.room_id !== r.id);
          db.damage_flags = db.damage_flags.filter((f) => f.room_id !== r.id);
        }
      }
